...
public class GraphManager : MonoBehaviour
{
    ...
    public class GPUGraphData
    {
        ...
        public void UpdateStats(float dataPoint, float minValue = float.NegativeInfinity, float maxValue = float.PositiveInfinity)
        {
            ResetDefault();
        ...
        public void AddPair(float dataPoint, Color color, Rect rectangle, float minValue=float.NegativeInfinity, float maxValue = float.PositiveInfinity)
        {
            ...
            Shrink();
            UpdateStats(dataPoint,minValue, maxValue);
            UpdateUI();
        }
        public void AddPair(float dataPoint, Color color, RectTransform rectTransform)
        {
            // each graph is located in a specify position // added function by Boxi 
            Rectangle = new Vector4(rectTransform.anchoredPosition.x / Screen.width, -rectTransform.anchoredPosition.y / Screen.height,
               rectTransform.sizeDelta.x * rectTransform.lossyScale.x / Screen.width, rectTransform.sizeDelta.y * rectTransform.lossyScale.y / Screen.height);
        ...
        //private Rect InitializeRect(RectTransform rectTransform)
        //{ // helper to get the Rect from RectTransform, for screen space plot
        //    float xSize = rectTransform.sizeDelta.x * rectTransform.lossyScale.x;
        //    float ySize = rectTransform.sizeDelta.y * rectTransform.lossyScale.y;
        ...
        public void UpdateText()
        {
            if (MaxText)
            {
                MaxText.text = string.Format("{0,7:###0.00}", MaxValue);
            }

            if (MinText)
            {
                MinText.text = string.Format("{0,7:###0.00}", MinValue);
            }

            if (AvgText)
            {
                AvgText.text = string.Format("{0,7:###0.00}", AverageValue);
            }
        }
    }
    ...
    public class GraphManagerInstance
    {
        ...
        // screen space plot
        public void Plot(string key, float value, Color color, Rect rectangle, float minValue = float.NegativeInfinity, float maxValue = float.PositiveInfinity)
        {
            Retrieve(key, value).AddPair(value, color, rectangle, minValue, maxValue);
        }
        public void Plot(string key, float value, Color color, RectTransform rectTransform)
        {
            //Rect rectangle = InitializeRect(rectTransform);
            Retrieve(key, value).AddPair(value, color, rectTransform);
        }
